...
class OlaTableCell extends BodyComponent {
  ...
  render() {
    ...
    const background_cell = is_highlight
      ? tokens(this.getAttribute('background-color'))
      : 'transparent';
    return `
            ${
              header
                ? `<th align="${this.getAttribute(
                    'align'
                  )}" style="padding: 15px; width: ${this.getAttribute(
                    'width'
                  )}">
                            ${this.renderChildren()}
                        </th>`
                : `<td align="${this.getAttribute(
                    'align'
                  )}" style="padding: ${this.getAttribute(
                    'padding'
                  )}; background-color: ${background_cell}; width: ${this.getAttribute(
                    'width'
                  )}">
                            ${this.renderChildren()}
                        </td>`
